...
#define PI 3.14159
/* ---------------------------------------------------------------------- */
FixUCG_GFRD::FixUCG_GFRD(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp,narg,arg)
{
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::find_initialstatedistribution()
{
...
/*------------------------------------------------------------------------*/
void FixUCG_GFRD::consistencycheck()
{
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::init_list(int id, NeighList *ptr)
{
  list = ptr;
}

/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::setup(int vflag)
{
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixUCG_GFRD::normalize(double *a)
{
  double mag=0.0;
  for(int k=0;k<3;k++)
    mag += a[k]*a[k];
...
void FixUCG_GFRD::computecross(double *a, double *b1, double *b2)
{
...
double FixUCG_GFRD::computedot(double *b1, double *b2)
{
...
double FixUCG_GFRD::computedihedral(double *x1, double *x2, double *x3, double *x4)
{
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::rotatepointalongaxisbyangle(double *q, double *p, double theta, double *r)
{
...
void FixUCG_GFRD::computemoldesum()
{
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::post_force(int vflag)
{
  //VARIABLES//
  ...
  if(change_flag==1)
    {
      ...
      if(accept_flag==1)
        {
	  next_reneighbor = update->ntimestep+1; //Trigger reneighboring at the next timestep. This is required since atom types are updated here.//
...
/* ---------------------------------------------------------------------- */
int FixUCG_GFRD::pack_forward_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int *type = atom->type;
  double **x = atom->x;
  int m=0,k;
...
/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::unpack_forward_comm(int n, int first, double *buf)
{
  int last=first+n;
  int m=0;
  int *type = atom->type;
...
/* ---------------------------------------------------------------------- */
double FixUCG_GFRD::memory_usage()
{
  return 0;
}

/* ---------------------------------------------------------------------- */
void FixUCG_GFRD::reset_accumulators()
{
  for(int imol=0;imol<nmol+1;imol++)
    {
      trans_flag[imol]=0;
...
/*------------------------------------------------------------------------*/
int FixUCG_GFRD::getnewatomtype(int imol, int itype, int ispecies)
{
